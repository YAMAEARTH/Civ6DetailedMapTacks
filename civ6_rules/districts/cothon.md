...
+50% [ICON_PRODUCTION] Production towards naval units and Settlers in this city. All wounded naval units in this city’s borders heal +100 HP each turn.

---

## 1. ข้อมูลพื้นฐานและการปลดล็อก (Basic Information & Unlocking)

- **DistrictType**: `DISTRICT_COTHON`
- **ชื่อภาษาอังกฤษ (Name)**: Cothon
- **ประเภทเขต (District Category)**: Specialty District (นับโควตาประชากรเมือง Pop 1, 4, 7, 10...)
- **แทนที่เขตมาตรฐาน (Replaces)**: `DISTRICT_HARBOR` (Harbor)
- **อารยธรรม / ผู้นำเฉพาะ (Exclusive to)**: Phoenicia (`TRAIT_CIVILIZATION_DISTRICT_COTHON`)
- **เทคโนโลยีที่ต้องการ (PrereqTech)**: Celestial Navigation (`TECH_CELESTIAL_NAVIGATION`)
...
| **Domain / Water** | `Water / Coast` | Water / Coast (ช่องน้ำชายฝั่ง) (ต้องติดแผ่นดิน) |
...
- **ภูมิประเทศที่อนุญาต (Valid Terrains)**: ช่องน้ำชายฝั่ง (`TERRAIN_COAST`) ที่ติดกับแผ่นดิน
...
| Gold | `+2` | เขต City Center | `DISTRICT_CITY_CENTER` | `Harbor_City_Gold` |
| Gold | `+1` | +{1_num} [ICON_Gold] Gold from the adjacent sea {1_Num : plural 1?resource; other?resources;}. | `None` | `SeaResource_Gold` |
...
```lua
-- การตรวจสอบความถูกต้องสำหรับการวาง Cothon (DISTRICT_COTHON)
function IsValidPlotFor_DISTRICT_COTHON(pPlayer, pPlot, pCity)
    ...
    if not pPlot:IsWater() or pPlot:GetTerrainType() ~= GameInfo.Terrains['TERRAIN_COAST'].Index then
        return false, 'MUST_BE_COAST_WATER';
    end
    if not IsAdjacentToLandPlot(playerID, px, py) then
        return false, 'MUST_BE_ADJACENT_TO_LAND';
    end
