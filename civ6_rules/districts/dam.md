...
> **คำอธิบายในเกม:**  
> A district built on a Floodplains tile to improve the comfort of living along this river. Prevents damage from flooding of this river (though yields from flooding drop by 50%). Prevents [ICON_Food] Food loss during Drought. Only one may be built along each river's floodplain (whichever player completes one first) and the river must traverse at least two sides of the Dam hex. Military Engineers can spend a charge to complete 20% of a Dam's production.

---

## 1. ข้อมูลพื้นฐานและการปลดล็อก (Basic Information & Unlocking)

- **DistrictType**: `DISTRICT_DAM`
- **ชื่อภาษาอังกฤษ (Name)**: Dam
...
- **เทคโนโลยีที่ต้องการ (PrereqTech)**: Buttress (`TECH_BUTTRESS`)
- **วัฒนธรรมที่ต้องการ (PrereqCivic)**: None (ไม่มี)
- **ต้นทุนการผลิตพื้นฐาน (Base Production Cost)**: 81 [ICON_Production] (สเกลตามความก้าวหน้า Tech/Civic สูงสุดถึง x10)
- **ค่าบำรุงรักษา (Maintenance)**: 0 [ICON_Gold] ทอง/เทิร์น
- **ที่อยู่อาศัย (Housing)**: +3
...
| **OnePerRiver (Dam)** | `true` | 1 แห่งต่อระบบแม่น้ำทั้งแผนที่ (ต้องเป็น Floodplains และมีแม่น้ำพาดผ่าน >= 2 ขอบช่อง) |
...
- **ฟีเจอร์ที่บังคับต้องมี (Required Features)**:
  - `FEATURE_FLOODPLAINS` (Desert Floodplains)
  - `FEATURE_FLOODPLAINS_GRASSLAND` (Grassland Floodplains)
  - `FEATURE_FLOODPLAINS_PLAINS` (Plains Floodplains)
...
```lua
-- การตรวจสอบความถูกต้องสำหรับการวาง Dam (DISTRICT_DAM)
function IsValidPlotFor_DISTRICT_DAM(pPlayer, pPlot, pCity)
    ...
    local requiredFeatures = {GameInfo.Features['FEATURE_FLOODPLAINS'].Index, GameInfo.Features['FEATURE_FLOODPLAINS_GRASSLAND'].Index, GameInfo.Features['FEATURE_FLOODPLAINS_PLAINS'].Index};
    local fIdx = pPlot:GetFeatureType();
    local hasReqFeature = false;
    for _, val in ipairs(requiredFeatures) do if fIdx == val then hasReqFeature = true; break; end end
    if not hasReqFeature then return false, 'MISSING_REQUIRED_FEATURE'; end

    -- 7. ตรวจสอบกฎการประชิดเมือง (City Center Adjacency)
    -- Dam Rule: 1 per river system & River must touch at least 2 edges of plot
    if IsDamAlreadyOnRiver(playerID, px, py) then return false, 'DAM_ALREADY_EXISTS_ON_RIVER'; end
    if CountRiverEdges(px, py) < 2 then return false, 'DAM_REQUIRES_AT_LEAST_TWO_RIVER_EDGES'; end
