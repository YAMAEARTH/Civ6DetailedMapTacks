# Acropolis (`DISTRICT_ACROPOLIS`)

> **คำอธิบายในเกม:**  
> A district unique to Greece for cultural sites. Replaces the Theater Square district and cheaper to build. Awards 1 [ICON_Envoy] Envoy when completed.

+1  [ICON_Culture] Culture bonus for each adjacent district and an additional +1 [ICON_Culture] Culture bonus for adjacent City Center. +2 [ICON_Culture] Culture bonus for each adjacent wonder, Entertainment Complex, and Water Park. Can only be built on Hills.
...
- **ชื่อภาษาอังกฤษ (Name)**: Acropolis
- **ประเภทเขต (District Category)**: Specialty District (นับโควตาประชากรเมือง Pop 1, 4, 7, 10...)
- **แทนที่เขตมาตรฐาน (Replaces)**: `DISTRICT_THEATER` (Theater Square)
- **อารยธรรม / ผู้นำเฉพาะ (Exclusive to)**: Greece (`TRAIT_CIVILIZATION_DISTRICT_ACROPOLIS`)
- **เทคโนโลยีที่ต้องการ (PrereqTech)**: None (ไม่มี)
- **วัฒนธรรมที่ต้องการ (PrereqCivic)**: Drama and Poetry (`CIVIC_DRAMA_POETRY`)
...
|---|---|---|---|---|
| Culture | `+1` | เขต City Center | `DISTRICT_CITY_CENTER` | `District_Culture_City_Center` |
| Culture | `+1` | เขตใดๆ (Any District) | `DISTRICT_ALL` | `District_Culture_Standard` |
| Culture | `+2` | เขต Entertainment Complex | `DISTRICT_ENTERTAINMENT_COMPLEX` | `EntertainmentComplex_Culture` |
| Culture | `+1` | เขต Government Plaza | `DISTRICT_GOVERNMENT` | `Government_Culture` |
| Culture | `+2` | ฟีเจอร์ Pamukkale | `FEATURE_PAMUKKALE` | `Pamukkale_Culture` |
| Culture | `+2` | เขต Water Park | `DISTRICT_WATER_ENTERTAINMENT_COMPLEX` | `WaterPark_Culture` |
| Culture | `+2` | สิ่งมหัศจรรย์ของโลก (World Wonder) | `WONDER` | `Wonder_Culture` |
...
```lua
-- การตรวจสอบความถูกต้องสำหรับการวาง Acropolis (DISTRICT_ACROPOLIS)
function IsValidPlotFor_DISTRICT_ACROPOLIS(pPlayer, pPlot, pCity)
