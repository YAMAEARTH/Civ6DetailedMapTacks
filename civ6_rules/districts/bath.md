...
It provides this city with a source of fresh water from an adjacent River, Lake, Oasis, or Mountain. Cities that do not yet have existing fresh water receive up to 6 [ICON_Housing] Housing. Cities that already have existing fresh water will instead get +2 [ICON_Housing] Housing. If built adjacent to a Geothermal Fissure +1 [ICON_Amenities] Amenity. In all cases the Bath provides an additional bonus of +2 [ICON_Housing] Housing and +1 [ICON_Amenities] Amenity. Prevents [ICON_Food] Food loss during Drought. Must be built adjacent to the City Center. Military Engineers can spend a charge to complete 20% of a Bath's production.

---

## 1. ข้อมูลพื้นฐานและการปลดล็อก (Basic Information & Unlocking)

- **DistrictType**: `DISTRICT_BATH`
- **ชื่อภาษาอังกฤษ (Name)**: Bath
- **ประเภทเขต (District Category)**: Non-Specialty District (ไม่จำกัดโควตา Pop / สร้างได้อิสระ)
- **แทนที่เขตมาตรฐาน (Replaces)**: `DISTRICT_AQUEDUCT` (Aqueduct)
- **อารยธรรม / ผู้นำเฉพาะ (Exclusive to)**: Rome (`TRAIT_CIVILIZATION_DISTRICT_BATH`)
- **เทคโนโลยีที่ต้องการ (PrereqTech)**: Engineering (`TECH_ENGINEERING`)
- **วัฒนธรรมที่ต้องการ (PrereqCivic)**: None (ไม่มี)
- **ต้นทุนการผลิตพื้นฐาน (Base Production Cost)**: 18 [ICON_Production] (สเกลตามความก้าวหน้า Tech/Civic สูงสุดถึง x10)
- **ค่าบำรุงรักษา (Maintenance)**: 0 [ICON_Gold] ทอง/เทิร์น
- **ที่อยู่อาศัย (Housing)**: +2
...
| **Aqueduct Rule** | `true` | ต้องติด City Center (Dist = 1) และติด River, Lake, Oasis, หรือ Mountain |
...
```lua
-- การตรวจสอบความถูกต้องสำหรับการวาง Bath (DISTRICT_BATH)
function IsValidPlotFor_DISTRICT_BATH(pPlayer, pPlot, pCity)
    ...
    if distFromCity ~= 1 then
        return false, 'AQUEDUCT_MUST_BE_ADJACENT_TO_CITY_CENTER';
    end
    if not IsValidAqueductWaterSource(pPlot) then
        return false, 'AQUEDUCT_REQUIRES_RIVER_LAKE_OASIS_OR_MOUNTAIN';
    end
