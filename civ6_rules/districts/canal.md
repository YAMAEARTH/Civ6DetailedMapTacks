...
- **เทคโนโลยีที่ต้องการ (PrereqTech)**: Steam Power (`TECH_STEAM_POWER`)
...
| **Canal Rule** | `true` | ที่ราบ เชื่อม 2 แหล่งน้ำ หรือ 1 น้ำ + City Center (ห้ามคลอง 3 ทาง / Endpoints dist >= 2) |
...
- **ภูมิประเทศที่อนุญาตเฉพาะ (Valid Terrains Only)**:
  ...
  - ⚠️ **กฎบังคับ**: ต้องสร้างบน **ที่ราบ (Flat Land Only)** เท่านั้น ห้ามเนินเขาเด็ดขาด!
...
```lua
-- การตรวจสอบความถูกต้องสำหรับการวาง Canal (DISTRICT_CANAL)
function IsValidPlotFor_DISTRICT_CANAL(pPlayer, pPlot, pCity)
    ...
    local validTerrains = {GameInfo.Terrains['TERRAIN_DESERT'].Index, GameInfo.Terrains['TERRAIN_GRASS'].Index, GameInfo.Terrains['TERRAIN_PLAINS'].Index, GameInfo.Terrains['TERRAIN_SNOW'].Index, GameInfo.Terrains['TERRAIN_TUNDRA'].Index};
    ...
    if pPlot:IsHills() then return false, 'FLAT_LAND_ONLY_NO_HILLS'; end

    -- 6. ตรวจสอบฟีเจอร์บังคับ (Required Features)

    -- 7. ตรวจสอบกฎการประชิดเมือง (City Center Adjacency)
    -- Canal Rule: Exactly 2 connectable endpoints (Water + Water or Water + City Center)
    local connectableCount = CountCanalConnectableEndpoints(px, py, pCity:GetX(), pCity:GetY());
    if connectableCount ~= 2 then
        return false, 'CANAL_EXACTLY_TWO_ENDPOINTS_NO_THREE_WAY';
    end
