# Hansa (`DISTRICT_HANSA`)

> **คำอธิบายในเกม:**  
> A district unique to Germany for industrial activity. Replaces the Industrial Zone district and cheaper to build.

+ 2[ICON_Production] Production bonus for each adjacent Commercial Hub, Aqueduct, Canal, and Dam districts. +1 [ICON_Production] Production bonus for each adjacent resource. +1 [ICON_Production] Production bonus for every two adjacent district tiles.
...
- **ชื่อภาษาอังกฤษ (Name)**: Hansa
- **ประเภทเขต (District Category)**: Specialty District (นับโควตาประชากรเมือง Pop 1, 4, 7, 10...)
- **แทนที่เขตมาตรฐาน (Replaces)**: `DISTRICT_INDUSTRIAL_ZONE` (Industrial Zone)
- **อารยธรรม / ผู้นำเฉพาะ (Exclusive to)**: Germany (`TRAIT_CIVILIZATION_DISTRICT_HANSA`)
...
| Production | `+2` | เขต Commercial Hub | `DISTRICT_COMMERCIAL_HUB` | `Commerical_Hub_Production` |
...
| Production | `+1` | ทรัพยากรทางทะเล (Sea Resource) | `SEA_RESOURCE` | `Resource_Production` |
...
```lua
-- การตรวจสอบความถูกต้องสำหรับการวาง Hansa (DISTRICT_HANSA)
function IsValidPlotFor_DISTRICT_HANSA(pPlayer, pPlot, pCity)
