# Mbanza (`DISTRICT_MBANZA`)

> **คำอธิบายในเกม:**  
> A district unique to Kongo that can only be constructed in Rainforest or Woods. Replaces the Neighborhood district but is available earlier and cheaper to build. Provides +5 [ICON_Housing] Housing , +2 [ICON_Food] Food, and +4 [ICON_Gold] Gold, regardless of Appeal.
...
- **อารยธรรม / ผู้นำเฉพาะ (Exclusive to)**: Kongo (`TRAIT_CIVILIZATION_MBANZA`)
- **เทคโนโลยีที่ต้องการ (PrereqTech)**: None (ไม่มี)
- **วัฒนธรรมที่ต้องการ (PrereqCivic)**: Guilds (`CIVIC_GUILDS`)
...
```lua
-- การตรวจสอบความถูกต้องสำหรับการวาง Mbanza (DISTRICT_MBANZA)
function IsValidPlotFor_DISTRICT_MBANZA(pPlayer, pPlot, pCity)
    ...
    local requiredFeatures = {GameInfo.Features['FEATURE_FOREST'].Index, GameInfo.Features['FEATURE_JUNGLE'].Index};
