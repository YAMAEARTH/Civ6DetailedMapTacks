# Thành (`DISTRICT_THANH`)

> **คำอธิบายในเกม:**  
> A district unique to Vietnam which replaces the Encampment. +2 [ICON_CULTURE] Culture for each adjacent district. After Flight is researched receive [ICON_TOURISM] Tourism equal to the [ICON_CULTURE] Culture output. This district does not require population, is cheaper to build, cannot be built adjacent to the city center, does not provide [ICON_GreatPerson] Great People points, and is not a specialty district.

---

## 1. ข้อมูลพื้นฐานและการปลดล็อก (Basic Information & Unlocking)

- **DistrictType**: `DISTRICT_THANH`
- **ชื่อภาษาอังกฤษ (Name)**: Thành
- **ประเภทเขต (District Category)**: Non-Specialty District (ไม่จำกัดโควตา Pop / สร้างได้อิสระ)
- **แทนที่เขตมาตรฐาน (Replaces)**: `DISTRICT_ENCAMPMENT` (Encampment)
- **อารยธรรม / ผู้นำเฉพาะ (Exclusive to)**: Vietnam (`TRAIT_CIVILIZATION_DISTRICT_THANH`)
- **เทคโนโลยีที่ต้องการ (PrereqTech)**: Bronze Working (`TECH_BRONZE_WORKING`)
...
| **NoAdjacentCity** | `true` | ห้ามสร้างติดกับ City Center เด็ดขาด (Distance >= 2) |
...
|---|---|---|---|---|
| Culture | `+2` | เขตใดๆ (Any District) | `DISTRICT_ALL` | `District_Culture_Major` |
...
```lua
-- การตรวจสอบความถูกต้องสำหรับการวาง Thành (DISTRICT_THANH)
function IsValidPlotFor_DISTRICT_THANH(pPlayer, pPlot, pCity)
    ...
    if distFromCity < 2 then
        return false, 'NO_ADJACENT_CITY_REQUIRED_DIST_GE_2';
    end
